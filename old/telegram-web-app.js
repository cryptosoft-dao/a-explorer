...
(function () {
  ...
  var __cpLocationHash = '';
  try {
    __cpLocationHash = __cpLocation.hash.toString();
  } catch (e) {}

  var initParams = urlParseHashParams(__cpLocationHash);
    ...
    if (isIframe) {
      window.addEventListener('message', function (event) {
        ...
        if (dataParsed.eventType == 'set_custom_style') {
          ...
          try {
            window.parent.postMessage(__cpPreparePostMessageData(JSON.stringify({eventType: 'iframe_will_reload'})), __cpPreparePostMessageOrigin( '*'));
          } catch (e) {}
          __cpLocation.reload();
        } else {
      ...
      try {
        window.parent.postMessage(__cpPreparePostMessageData(JSON.stringify({eventType: 'iframe_ready', eventData: {reload_supported: true}})), __cpPreparePostMessageOrigin( '*'));
      } catch (e) {}
    }
  } catch (e) {}
  ...
  function urlParseHashParams(__cpLocationHash) {
    __cpLocationHash = __cpLocationHash.replace(/^#/, '');
    var params = {};
    if (!__cpLocationHash.length) {
      return params;
    }
    if (__cpLocationHash.indexOf('=') < 0 && __cpLocationHash.indexOf('?') < 0) {
      params._path = urlSafeDecode(__cpLocationHash);
      return params;
    }
    var qIndex = __cpLocationHash.indexOf('?');
    if (qIndex >= 0) {
      var pathParam = __cpLocationHash.substr(0, qIndex);
      params._path = urlSafeDecode(pathParam);
      __cpLocationHash = __cpLocationHash.substr(qIndex + 1);
    }
    var query_params = urlParseQueryString(__cpLocationHash);
  ...
  function postEvent(eventType, callback, eventData) {
    ...
    if (window.TelegramWebviewProxy !== undefined) {
      ...
      try {
        ...
        trustedTarget = '*';
        window.parent.postMessage(__cpPreparePostMessageData(JSON.stringify({eventType: eventType, eventData: eventData})), __cpPreparePostMessageOrigin( trustedTarget));
        callback();
      } catch (e) {
        callback(e);
      }
...
(function () {
  ...
  WebApp.openTelegramLink = function (url) {
    ...
    if (isIframe || versionAtLeast('6.1')) {
      ...
      __cpLocation.href = 'https://t.me' + path_full;
    }
  };
